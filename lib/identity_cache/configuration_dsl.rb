  ...
  module ConfigurationDSL
    ...
    module ClassMethods
      ...
      def identity_cache_single_value_dynamic_fetcher(fields, values) # :nodoc:
        sql_on_miss = "SELECT #{quoted_id_column} FROM `#{table_name}` WHERE #{identity_cache_sql_conditions(fields, values)} LIMIT 1"
      ...
      def identity_cache_multiple_value_dynamic_fetcher(fields, values) # :nodoc
        sql_on_miss = "SELECT #{quoted_id_column} FROM `#{table_name}` WHERE #{identity_cache_sql_conditions(fields, values)}"
        cache_key = rails_cache_index_key_for_fields_and_values(fields, values)
