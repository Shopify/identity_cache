...
class FetchTest < IdentityCache::TestCase
  ...
  def test_exists_with_identity_cache_when_cache_miss_and_in_db
    ...
    Item.send(:cached_record_fetcher).expects(:load_from_db).with(1).once.returns(@record)
  ...
  def test_exists_with_identity_cache_when_cache_miss_and_not_in_db
    fetch = Spy.on(IdentityCache.cache, :fetch).and_call_through
    Item.send(:cached_record_fetcher).expects(:load_from_db).with(1).once.returns(nil)
  ...
  def test_fetch_conflict
    load_from_db = Spy.on(Item.send(:cached_record_fetcher), :load_from_db).and_return do
      @record.expire_cache
      @record
    end
    ...
    assert_equal(@record, Item.fetch(1))
    assert(load_from_db.has_been_called_with?(1))
